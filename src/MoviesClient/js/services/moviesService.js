services.factory('moviesService', ['$http', '$q', function ($http, $q) {
	...
	function updateMovie(movieItem, savedCallback) {
		$http.post('/service/MoviesService.svc/json/movies/update', getServerItem(movieItem)).then(function (response) {
			if (savedCallback) {
				savedCallback();
			}
		});
	...
	function getMovie(id, callback) {
		...
		$http.get('/service/MoviesService.svc/json/movies/' + idParam).success(function (response) {
